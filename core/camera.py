...
class ThreadedCamera:
    def __init__(self, src=0):
        ...
        self.src_main = src.strip() if isinstance(src, str) else src
        ...
        # Determine self.src_sub dynamically
        self.src_sub = self.src_main
        if isinstance(self.src_main, str) and "://" in self.src_main:
            if "/main" in self.src_main:
                self.src_sub = self.src_main.replace("/main", "/sub")
            elif "subtype=0" in self.src_main:
                self.src_sub = self.src_main.replace("subtype=0", "subtype=1")
            elif "stream=0" in self.src_main:
                self.src_sub = self.src_main.replace("stream=0", "stream=1")
            elif "/ch1/1" in self.src_main:
                self.src_sub = self.src_main.replace("/ch1/1", "/ch1/2")
        ...
        if self.is_alive:
            ...
            # 2. Open Main-stream (grab-only, decode on retrieve snapshot demand)
            if self.src_main != self.src_sub and isinstance(self.src_main, str) and "://" in self.src_main:
                print(f"[CAMERA] Opening Main-stream (Grab-only): {self.src_main}")
                ...
                if self.capture_main and self.capture_main.isOpened():
                    ...
                    print(f"[CAMERA] Main-stream CAM_{self.src_main} OPENED SUCCESSFULLY")
                else:
                    print(f"[CAMERA ERROR] FAILED to open main-stream source: {self.src_main}")
        else:
