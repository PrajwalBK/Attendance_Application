...
class FaceRecognitionHandler:
    ...
    def extract_upper_face_encoding(self, frame, face_bbox):
        ...
        try:
            ...
            if embedding is None:
                return None
            raw_emb = embedding[0] if embedding.ndim == 2 else embedding
    ...
    def recognize_multiple_faces_masked(self, faces, frame=None):
        ...
        for face in faces:
            ...
            # Quality check: Skip tiny face crops (< 30px)
            ...
            # Standard full-face recognition attempt (strictly enforces SIMILARITY_THRESHOLD)
            person_id, person_name, similarity = self.recognize_face(face.embedding)
            is_masked = False
            recognition_method = 'full_face' if person_id else None
            
            # If standard recognition FAILED, try upper-face mask-aware fallback
            if person_id is None and self.mask_detection_enabled and frame is not None:
                if similarity >= 0.10:
                    ...
                    if is_masked:
                        # Attempt upper-face crop extraction & direct upper-face matching
